...
function User({userContents, setUserContents}) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const response = await fetch("https://capstone-project-fitness-logger1.onrender.com/api/content/user/all", {
        method: "GET",
        headers: {
  ...
  const handleDelete = async (id) => {
    const res = await fetch(`https://capstone-project-fitness-logger1.onrender.com/api/content/user/delete/${id}`, {
      method: "DELETE",
      headers: {
  ...
  return (
    <>
        ...
        <div className='container'>
            <h1 className='text-center mt-2'>My Own Exercises & Posts</h1>
        </div>

        <div className='text-center'>
          <button className='btn btn-outline-success px-3 m-1' onClick={() => navigate("/addcontents")}>Add New Exercises For You</button>
        </div>
        ...
        <div className="container">
          {userContents?.map((data) => (
            <div  key={data._id} className='border rounded py-2 mt-2 w-75 mx-auto text-center' style={{backgroundColor: "OldLace"}}>
              ...
              <div>
                <img src={`https://capstone-project-fitness-logger1.onrender.com/${data.file}`} alt="" style={{width: "70%", height: "auto", padding: "1rem"}}/>
              </div>
